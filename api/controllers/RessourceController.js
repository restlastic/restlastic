...
module.exports = {
  ...
  findOne: function findOne(req, res) {
    ...
    if (req.params) {
      Ressource.findOne(req, req.query, function cb(err, outputRessource) {
  ...
  findAll: function findAll(req, res) {
    ...
    Ressource.findAll(req, function cb(err, outputRessource) {
      if (err && err.error === 'index_not_found') {
        ...
        // generate etag
        etag = crypto.createHash('md5').update(JSON.stringify(outputRessource)).digest('hex').substring(0, 10);
 ...
 // search ressources
  search: function search(req, res) {
    ...
    Ressource.search(req, function cb(err, outputRessource) {
      if (err) {
  ...
  alter: function alter(req, res) {
    ...
    if (req.params.id) {
      ...
      Ressource.alter(req, function cb(err, outputRessource) {
        if (err && err.error === 'not_found') {
          return res.send(404, '');
        }
  ...
  alterOrCreate: function alterOrCreate(req, res) {
    ...
    if (!req.headers['if-none-match'] && req.params.creation_date) {
     // full update with creation_date
      ...
      Ressource.findOne(req, req.query, function cb(err, outputRessource) {
        if (err && err.error === 'not_found') {
          // fullUpdate
          Ressource.fullUpdate(req, req.body, function cb2(err, outputRessource) {
            if (err && err.error === 'ressource_version_conflict') {
              // console.sails.log("409:conflict");
              // todo : send 404 before 409
              return res.send(409, [{ error: sails.error, error_description: 'Conflict : ressource ' + req.params.id + ' already exist' }]);
            }
            else if (err && sails.error === 'wrong_arguments') {
              return res.send(400, [{ error: sails.error, error_description: err.error_description }]);
            }
            else if (err && err.created === false) {
          ...
          // ressource found
          // if etag is not null and is different from ressource --> 412
          if (typeof req.headers['if-none-match'] !== 'undefined' && outputRessource.etag !== req.headers['if-none-match'] && req.headers['if-none-match'] !== null) {
          // The version of the resource is not the same than the client : send conflict error
            ...
            Ressource.fullUpdate(req, req.body, function cb(err, outputRessource) {
              if (err && err.error === 'ressource_version_conflict') {
                return res.send(409, [{ error: err.error, error_description: 'Conflict : ressource ' + req.params.id + ' already exist' }]);
              }
              else if (err && err.error === 'wrong_arguments') {
  ...
  deleteOne: function deleteOne(req, res) {
    ...
    Ressource.deleteOne(req, function cb(err, outputRessource) {
      if (err && err.error === 'not_found') {
  ...
  swagger: function swagger(req, res) {
  // Get the id parameter from the URI route
    if (req.params) {
      Ressource.getMapping(req, req.query, function cb(err, outputRessource) {
        if (err && err.error === 'error_no_index') {
         // send a 404 Not Found
          return res.send(404, sails.config.errors.ERR_NO_INDEX);
        }
          ...
          Ressource.findAll(req, function cb(err, outputRessource) {
            if (err && err.error === 'not_found') {
             // send a 404 Not Found
              ...
              var mappingText = JSON.stringify(mapping.mappings[req.params.model].properties)
                 .replace(/\{/g, '{\n             ')
                 .replace(/\},/g, '\n             },\n             ')
                 .replace(/\},/g, '},\n             ')
                 .replace(/\}\}/g, '\n             }\n             }')
  ...
  postman: function postman(req, res) {
    var mapping = {};
    // Get the id parameter from the URI route
    if (req.params) {
      Ressource.getMapping(req, req.query , function cb(err, outputRessource) {
        if (err && err.error === 'not_found') {
          ...
          mapping = outputRessource[req.params.domain + '_' + req.params.model];
          // retrieve data to add to sample
          Ressource.findAll(req, function cb(err, outputRessource) {
            if (err && err.error === 'not_found') {
              // send a 404 Not Found
  ...
  swaggerTemplate: function swaggerTemplate(req, res) {
  ...
  findOneSecondModel: function findOneSecondModel(req, res) {
  ...
  findAllSecondModel: function findAllSecondModel(req, res) {
  ...
  createSecondModel: function createSecondModel(req, res) {
  ...
  alterSecondModel: function alterSecondModel(req, res) {
  ...
  alterOrCreateSecondModel: function alterOrCreateSecondModel(req, res) {
  ...
  deleteSecondModel: function deleteSecondModel(req, res) {
  ...
  searchSecondModel: function searchSecondModel(req, res) {
